# Global Bridge — Design System

This file is the single source of truth for the Global Bridge site.

It is built on the style reference below, which was captured from Awesomic. **Its
structure, geometry, typography scale, spacing and component rules all apply as
written — with one deliberate exception: the colors.** Global Bridge has its own
brand, taken from the company's print collateral (logo sheet, letterhead and
business cards). The zinc-grey palette and the orange accent in the reference are
replaced by the warm palette below. Everywhere the reference names a grey or the
orange, use its Global Bridge equivalent from this table.

## Color overrides — Global Bridge brand

| Token | Value | Replaces | Role |
|-------|-------|----------|------|
| `--color-espresso` | `#432818` | Obsidian `#09090b` | Primary action buttons, display headlines, dark surfaces |
| `--color-cocoa` | `#603824` | Deep Dark `#27272a` | Deeper dark surfaces and dark cards |
| `--color-oxblood` | `#6f1d1b` | Ember `#ff5a00` | The logo color. The single vivid accent — used sparingly, as punctuation |
| `--color-copper` | `#99582a` | — | Links, hover states, secondary accent |
| `--color-gold` | `#bb9457` | — | Subtitles, hairline rules, refined detail |
| `--color-sand` | `#d8cdb8` | Mist `#d4d4d8` | Secondary fills, dividers |
| `--color-taupe` | `#c9bda9` | Ash `#a1a1aa` | Watermarks, muted marks, disabled states |
| `--color-cream` | `#f4ebd7` | Subtle Card `#fafafa` | Recessed surfaces within content |
| `--color-paper` | `#f9f8f7` | Paper `#f4f4f5` | Canvas background |
| `--color-snow` | `#ffffff` | Snow `#ffffff` | Elevated card surfaces |
| `--color-ink` | `#191817` | Graphite `#18181b` | Primary text |
| `--color-stone` | `#6d645e` | Steel `#52525b` | Body text, supporting copy |
| `--color-linen` | `#e8e1d4` | Cloud `#ececee` | 1px hairline borders — the only permitted elevation on cards |

Every value above is lifted from the print collateral except `--color-linen`,
which is a warm tint derived between cream and sand to serve as the hairline
border the system requires.

**Typeface:** DM Sans, at every typographic role, as the substitute for Cosmica.
The single-font rule from the reference holds.

The canonical implementation of all of this lives in `src/app/globals.css`.
